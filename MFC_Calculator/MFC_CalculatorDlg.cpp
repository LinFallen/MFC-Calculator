...
BEGIN_MESSAGE_MAP(CMFCCalculatorDlg, CDialogEx)
	...
	ON_STN_CLICKED(IDC_STATIC_RESULT, &CMFCCalculatorDlg::OnStnClickedStaticResult)
	ON_COMMAND(ID_32785, &CMFCCalculatorDlg::ClearAll)
	ON_COMMAND(ID_32786, &CMFCCalculatorDlg::Delete)
	ON_COMMAND(ID_32780, &CMFCCalculatorDlg::Plus)
	ON_COMMAND(ID_32781, &CMFCCalculatorDlg::Minus)
	ON_COMMAND(ID_32782, &CMFCCalculatorDlg::Multiple)
	ON_COMMAND(ID_32783, &CMFCCalculatorDlg::Divide)
	ON_COMMAND(ID_32784, &CMFCCalculatorDlg::Equal)
	ON_COMMAND(ID_32779, &CMFCCalculatorDlg::InfoAbout)
END_MESSAGE_MAP()
...
BOOL CMFCCalculatorDlg::OnInitDialog()
{
	...
	m_Menu.LoadMenu(IDR_MENU2);  //  IDR_MENU2
	SetMenu(&m_Menu);
...
void CMFCCalculatorDlg::OnBnClickedButtonEqual()
{
	...
	GetDlgItem(IDC_EDIT_EXP)->GetWindowText(strExp);
	//std::string infix = strExp.GetBuffer(0);
	//std::string infix = LPCSTR(strExp);
	...
	*/
	...
	SetDlgItemText(IDC_EDIT_EXP, cstr_Result);
}
...
//结果输出文本区
void CMFCCalculatorDlg::OnStnClickedStaticResult()
{
	// TODO: 在此添加控件通知处理程序代码
}
...
void CMFCCalculatorDlg::Multiple()
{
	// TODO: 在此添加命令处理程序代码
	AddToEditExp(IDC_BUTTON_MUL);
}
...
void CMFCCalculatorDlg::InfoAbout()
{
	// TODO: 在此添加命令处理程序代码
	InfoAboutDlg Dlg;
	Dlg.DoModal();
